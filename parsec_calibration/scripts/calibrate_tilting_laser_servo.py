...
LASER_DISTANCE_FROM_ROTATION_AXIS = 0.033
    ...
    with self._lock:
      if len(self._tilt_signals) != 3:
        raise CalibrationError(
            'Invalid number of signals. We need at exactly three signals.')
      if (self._tilt_signals[0].signal != parsec_msgs.LaserTiltSignal.ANGLE_INCREASING or
        ...
        raise CalibrationError('Invalid signals. The first and last signal need to be ANGLE_INCREASING.')
      if self._scans.get_newest_scan().header.stamp < self._tilt_signals[-1].header.stamp:
        raise CalibrationError('Newest scan is older than newest signal.')
      if self._scans.get_oldest_scan().header.stamp > self._tilt_signals[0].header.stamp:
        raise CalibrationError('Oldest scan is newer than oldest signal.')
      ...
      scan_parameter_finder = find_scan_parameters.FindScanParameters(
          sensor_distance_from_rotation_axis=LASER_DISTANCE_FROM_ROTATION_AXIS)
      parameters = scan_parameter_finder.find_scan_parameters(
          [scan.ranges[int(len(scan.ranges) / 2)] for scan in scans],
          (end_time - start_time).to_sec() / self._tilt_period)
